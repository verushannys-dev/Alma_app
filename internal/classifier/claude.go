...
// outputConfig controla el "esfuerzo" de razonamiento que invierte el
// modelo antes de responder. Claude Sonnet 5 tiene pensamiento adaptativo
// activado por default, a "effort": "high" — pensado para tareas
// complejas de programación/razonamiento largo. Para una clasificación
// de mensajes cortos (lo que hace este Clasificador), la documentación
// oficial de Anthropic recomienda "low" explícito:
// "Use low for speed-sensitive or simple tasks... suitable for chat and
// non-coding use cases where faster turnaround is prioritized."
// (https://platform.claude.com/docs/en/build-with-claude/effort)
//
// IMPORTANTE: esto NO reduce la capacidad del modelo de interpretar
// lenguaje natural — sigue siendo el mismo LLM haciendo la misma
// interpretación semántica. Lo que cambia es cuánto "piensa antes de
// responder", apropiado para una tarea bien acotada como esta.
//
// NO se fija "temperature" acá a propósito: según la documentación
// oficial, los modelos posteriores a Claude Opus 4.6 (entre ellos
// claude-sonnet-5) rechazan con error 400 cualquier valor de
// temperature/top_p/top_k distinto del default — no es un parámetro
// disponible para controlar consistencia en este modelo.
...
func (c *ClaudeClassifier) ClassifyIntent(ctx context.Context, mensaje string, sessionCtx types.SessionContext, pending []types.Intent) (types.ClassificationResult, error) {
	...
	reqBody := claudeRequest{
		Model:        c.model,
		MaxTokens:    2000, // 1000 se quedaba corto — ver nota de tokenizer/thinking arriba
	...
	// No asumimos que el primer bloque es el texto — Claude Sonnet 5 tiene
	// pensamiento adaptativo activado por default, y ese razonamiento
	// puede venir como un bloque de contenido ANTES del bloque de texto.
