...
func (c *ClaudeClassifier) ClassifyIntent(ctx context.Context, mensaje string, sessionCtx types.SessionContext, pending []types.Intent) (types.ClassificationResult, error) {
	...
	// Asume que el primer bloque de texto es el JSON de clasificación.
	// TODO: si el modelo devuelve texto extra (preámbulo, backticks de markdown),
	// agregar acá un paso de limpieza antes del Unmarshal.
	if err := json.Unmarshal([]byte(claudeResp.Content[0].Text), &result); err != nil {
		return result, fmt.Errorf("el texto de Claude no es JSON válido de ClassificationResult: %w", err)
	}
