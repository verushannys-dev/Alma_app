...
func TestClasificador(t *testing.T) {
	...
	casos := []struct {
		...
		},
		{
			...
			verificar: func(t *testing.T, r types.ClassificationResult) {
				it := buscarPorDomain(r.Intents, "no_disponible")
				if it == nil {
					t.Fatalf("esperaba domain=no_disponible, recibí: %+v", r.Intents)
				}
				esperado := "Tu pedido aún no puede ser gestionado por Alma. ¿Puedo guiarte con algo más?"
				if it.Clarification != esperado {
					t.Errorf("clarification = %q, esperaba %q", it.Clarification, esperado)
				}
				for _, otro := range r.Intents {
					if otro.Domain != "turnos" && otro.Domain != "sedes" && otro.Domain != "no_disponible" {
						t.Errorf("el Clasificador devolvió un domain fuera de los 3 válidos: %q", otro.Domain)
					}
				}
			},
		},
