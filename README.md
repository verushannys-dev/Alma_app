...
- **Serie de bugs en `claude.go`, encontrados en la primera corrida real
  contra la API (28/08/2026) — diagnóstico completo, con fuentes.**
  Claude Sonnet 5 (el modelo que usa este proyecto) tiene comportamiento
  específico que versiones anteriores de Claude no tenían, documentado en
  https://platform.claude.com/docs/en/models/sonnet-5/whats-new-sonnet-5
  y https://platform.claude.com/docs/en/build-with-claude/effort — nada
  de esto estaba contemplado en el código original:
  1. **`temperature`, `top_p`, `top_k` no se pueden fijar** — cualquier
     valor no-default devuelve error 400. Se probó fijar `temperature: 0`
     pensando en consistencia, y se revirtió antes de correrlo en serio
     al confirmar esto en la documentación oficial.
  2. **Pensamiento adaptativo activado por default**, a `effort: "high"` —
     pensado para tareas complejas (programación, razonamiento largo), no
     para clasificar mensajes cortos. La respuesta de la API puede traer
     un bloque de "pensamiento" ANTES del bloque de texto — el código
     asumía que `Content[0]` siempre era el texto, y no siempre lo es.
     Corregido: se busca el/los bloques de tipo `"text"` explícitamente.
  3. **Tokenizador nuevo, ~30% más tokens para el mismo texto** — sumado
     a que el pensamiento interno también consume del límite de
     `max_tokens`, el corte a mitad de JSON que se vio al principio tiene
     más de una causa posible, no solo el límite bajo original (subido de
     1000 a 2000).
  4. **`effort` bajado explícito a `"low"`** — la documentación oficial
     de Anthropic recomienda este nivel para "tareas simples, sensibles a
     latencia... casos de uso de chat, no de programación" — exactamente
     este caso. No reduce la capacidad de interpretación semántica del
     modelo (eso no está en discusión), solo cuánto "piensa" antes de
     responder — apropiado para una tarea bien acotada como clasificar.
  5. **Falta de limpieza de backticks de markdown** — el prompt le pide a
     Claude que no los use, pero el modelo a veces los pone igual.
     Agregado `limpiarBackticksMarkdown()`.

  **Lo que este arreglo NO garantiza, dicho con la misma honestidad que
  el resto de este documento:** la propia documentación de Anthropic
  reconoce que ningún nivel de configuración (ni siquiera `temperature:
  0` en modelos que lo permiten) garantiza determinismo absoluto. Si
  después de este fix sigue apareciendo variación en casos genuinamente
  ambiguos, no es evidencia de un bug nuevo — hay que evaluarlo caso por
  caso, no asumir automáticamente que hace falta otro parche.
...
> ✅ **Etapa 1 (Clasificador) validada contra la API real el 28/08/2026 —
> 18/18 casos en verde.** Fue un camino con vueltas reales, documentadas
> arriba en "Gaps pendientes": un modelo (Claude Sonnet 5) con
> comportamiento nuevo que el código original no contemplaba (`effort`,
> pensamiento adaptativo, sampling bloqueado), un caso de prueba mal
> diseñado que escondía la causa real de una falla, y un gap genuino de
> diseño (`domain: "no_disponible"`) encontrado recién al probar con un
> mensaje fuera del catálogo actual. Ninguno de los tres se resolvió
> adivinando — cada uno con su propia verificación antes de dar el
> problema por resuelto. Próxima etapa: Orquestador (ver spec_orquestador.md
> sección B3 para el pendiente ya identificado de esa integración).
...
**Test en lote** (`internal/classifier/claude_test.go`) — 18 mensajes
