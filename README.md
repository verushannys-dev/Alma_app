...
- **Bug en investigación, dos rondas de arreglo — encontrado en la
  primera corrida real contra la API (28/08/2026).** Primera ronda:
  `MaxTokens` en 1000 se quedaba corto (subido a 2000), y faltaba
  limpiar backticks de markdown que el modelo agrega pese a que el
  prompt pide no hacerlo (agregado `limpiarBackticksMarkdown()`).
  Segunda ronda, tras volver a correr: el error cambió a "texto
  recibido: vacío" — el código asumía que `Content[0]` siempre era el
  bloque de texto, y aparentemente no siempre lo es (hipótesis: algún
  bloque de otro tipo, ej. razonamiento interno, puede venir primero).
  Corregido para buscar el/los bloques de tipo `"text"` en vez de leer
  `Content[0]` a ciegas, y el mensaje de error ahora informa qué tipos
  de bloque llegaron — si vuelve a fallar, esa lista dice exactamente
  qué está pasando en vez de tener que seguir adivinando.
